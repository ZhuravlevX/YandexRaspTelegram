...
def search_trains(from_city: str, to_city: str, date: str, tz: str):
...
def format_train_info(trains, info, date, tz_str):
    train_info = []
    msg = ""
...
def get_train_info(from_city: str, to_city: str, tz: str) -> str | None:
    ...
    trains, info = search_trains(from_city, to_city, date_str, tz)
    if trains is None:
        return None

    msg = format_train_info(trains, info, date, tz_str)
    if not msg:
        next_day = date + timedelta(days=1)
        next_day_str = next_day.strftime('%Y-%m-%d')
        trains, info = search_trains(from_city, to_city, next_day_str, tz)
        if trains is None:
            return None
        msg = format_train_info(trains, info, next_day, tz_str)
