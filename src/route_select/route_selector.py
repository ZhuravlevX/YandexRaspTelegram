...
from src.models.mosmetro.search_response_underground import SearchResponse, StationMini
...
async def from_station_handler(message: Message, state: FSMContext):
    ...
    if len(stations) == 0:
        ...
        response = await message.reply(
            ...
            reply_markup=(await select_stations_keyboard(stations, direction='from', state=state)))
        messages.append({'chat_id': response.chat.id, 'message_id': response.message_id})
        await state.update_data(messages=messages)
...
async def to_station_handler(message: Message, state: FSMContext):
    ...
    if len(stations) == 0:
        ...
        response = await message.reply(
            f'🏫🔍 <b>Найдены следующие станции с похожим названием:</b>',
            reply_markup=(await select_stations_keyboard(stations, direction='to', state=state)))
        messages.append({'chat_id': response.chat.id, 'message_id': response.message_id})
        await state.update_data(messages=messages)
...
async def from_city_handler(message: Message, state: FSMContext):
    ...
    if len(cities) == 0:
        ...
        response = await message.reply(
            ...
            reply_markup=(await select_cities_keyboard(cities, direction='from', state=state)))
        messages.append({'chat_id': response.chat.id, 'message_id': response.message_id})
        await state.update_data(messages=messages)
...
async def to_city_handler(message: Message, state: FSMContext):
    ...
    if len(cities) == 0:
        ...
        response = await message.reply(
            f'🏙🔍 <b>Найдены следующие города с похожим названием:</b>',
            reply_markup=(await select_cities_keyboard(cities, direction='to', state=state)))
        messages.append({'chat_id': response.chat.id, 'message_id': response.message_id})
        await state.update_data(messages=messages)
...
async def select_underground(station_id, direction, message: Message, state: FSMContext):
    station = StationMini(**requests.get(f'{os.getenv("BACKEND_URL")}{os.getenv("PORT")}/mosmetro/station/mini/{station_id}').json())
    data = await state.get_data()
...
async def select_from_underground(message: Message, state: FSMContext):
    search_res = requests.get(f'{os.getenv("BACKEND_URL")}{os.getenv("PORT")}/mosmetro/station/search?q={message.text}')
    data = await state.get_data()
