use crate::types::mostrans::troika::CardInfoResponse;
use crate::types::Environment;
use actix_web::{error, get, web, HttpResponse};

#[get("/troika/products")]
pub async fn get_troika_products(
    env: web::Data<Environment>,
    client: web::Data<awc::Client>,
) -> error::Result<HttpResponse> {
    let mut card_info_response = client
        .get(format!(
            "{}/cards/v1.0/14832563147930934/validate/payment",
            env.lk_mosmetro_api_url
        ))
        .insert_header((
            "User-Agent",
            "Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:137.0) Gecko/20100101 Firefox/137.0",
        ))
        .send()
        .await
        .map_err(error::ErrorBadGateway)?;

    let card_info_response_data = card_info_response
        .json::<CardInfoResponse>()
        .await
        .map_err(error::ErrorInternalServerError)?;

    if !card_info_response_data.success {
        return Err(error::ErrorInternalServerError("Card Info request failed"));
    };

    Ok(HttpResponse::Ok().json(card_info_response_data.data.available_products))
}
